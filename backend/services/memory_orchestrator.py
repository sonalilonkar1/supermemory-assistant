...
def build_context_for_turn(user_id: str, role: str, user_message: str) -> Dict[str, Any]:
    ...
    cross_role_static = build_cross_role_static(profile, role) if profile else {}
    ...
    cross_roles = get_cross_roles_for(role)
    cross_role_memories: List[Dict] = []
    if cross_roles:
        # Search other roles for relevant memories (strictly mode-scoped per role)
        merged: List[Dict] = []
        for r in cross_roles:
            try:
                merged.extend(search_memories(user_id=user_id, query=user_message, role=r, limit=6))
            except Exception:
                pass
